...
# Clean the data to ensure it includes only valid months
x_clean <- x %>%
  filter(month >= 1 & month <= 12)

# Aggregate data to get the total count of attacks per month across all years
monthly_attacks <- x_clean %>%
  group_by(month) %>%
  summarize(NumberOfAttacks = n())

# Convert month to a factor for the GLM
monthly_attacks$month <- as.factor(monthly_attacks$month)

# Fit a Poisson regression model (use negative binomial if overdispersion is present)
...
# Convert the date to a numeric value for regression
...
# Assuming x is your raw data with a 'year' column and 'month' is coded numerically 1-12
library(tidyverse)
